...
class Init {
	...
	public function notice_install_plugin() {
		...
		$button_text = __( 'Install Required Plugins', 'hypebiz' );
		...
		<style>
			.install-gutenverse-plugin-notice {
				...
				background: url( <?php echo esc_url( HYPEBIZ_URI . '/assets/img/background-banner.png' ); ?> );
		...
		jQuery( function( $ ) {
			$( 'div.notice.install-gutenverse-plugin-notice' ).on( 'click', 'button.notice-dismiss', function( event ) {
				...
				$.post( ajaxurl, {
					...
					nonce: '<?php echo esc_html( wp_create_nonce( 'hypebiz_admin_notice' ) ); ?>',
				} );
			} );
			...
			<div class="gutenverse-notice-inner">
				<div class="gutenverse-notice-content">
					<div class="gutenverse-notice-text">
						<h3><?php esc_html_e( 'Take Your Website To New Height with', 'hypebiz' ); ?> <span>Gutenverse!</span></h3> 
						<p><?php esc_html_e( 'Hypebiz theme work best with Gutenverse plugin. By installing Gutenverse plugin you may access Hypebiz templates built with Gutenverse and get access to more than 40 free blocks, hundred free Layout and Section.', 'hypebiz' ); ?></p>
					...
					<div class="gutenverse-notice-image">
						<img src="<?php echo esc_url( HYPEBIZ_URI . '/assets/img/banner-install-gutenverse-2.png' ); ?>"/>
					</div>
				</div>
			</div>
		</div>
	...
	public function backend_scripts() {
		...
		if ( $screen->id === 'appearance_page_hypebiz-dashboard' ) {
			...
			wp_localize_script( 'hypebiz-dashboard', 'GutenverseThemeConfig', $this->theme_config() );
		}
	}
	...
	public function theme_config() {
		return array(
			'demo'    => esc_url( 'https:/gutenverse.com/demo?name=hypebiz' ),
			'pages'   => array(
