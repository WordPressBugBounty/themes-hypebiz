...
class Plugin_Notice {
	...
	private function load_hooks() {
		...
		add_action( 'wp_ajax_hypebiz_dismiss_notice', array( $this, 'dismiss_notice' ) );
	}
	...
	public function notice_install_plugin() {
		...
		if ( get_option( 'hypebiz_dismiss_notice' ) ) {
			return;
		}

		$screen = get_current_screen();
		if ( isset( $screen->parent_file ) && 'themes.php' === $screen->parent_file && 'appearance_page_hypebiz-dashboard' === $screen->id ) {
			return;
		}
  ...
  '1x' => 'https://ps.w.org/gutenverse-companion/assets/icon-128x128.png?rev=3162415',
		...
		wp_register_style( 'hypebiz-theme-notice', false );
		wp_enqueue_style( 'hypebiz-theme-notice' );
		ob_start();
		?>
		
				.hypebiz-simple-notice {
				
					padding: 24px !important; 
				
					border-left: 4px solid #007cba !important;
				}
				.hypebiz-simple-notice p:first-child {
				
					margin-bottom: 20px; 
					margin-top: 0;
				}
				.hypebiz-simple-notice p:last-of-type {
				
					margin-top: 0; 
					margin-bottom: 0;
				}
				.hypebiz-simple-notice .hypebiz-notice-title {
					font-size: 17px;
					font-weight: 600; 
					display: block;
					margin-bottom: 6px;
				}
				.hypebiz-simple-notice .hypebiz-notice-description {
					font-size: 13px;
					display: block;
				...
				.hypebiz-simple-notice .button-primary {
					padding: 4px 16px !important;
				}

			
				.hypebiz-simple-notice .notice-dismiss {
					background: none !important;
					box-shadow: none !important;
					opacity: 1;
				}
				
				.hypebiz-simple-notice .notice-dismiss:before {
					color: #a7aaad;
					padding: 0; 
				}
				
				.hypebiz-simple-notice .notice-dismiss:hover:before,
				.hypebiz-simple-notice .notice-dismiss:focus:before {
					color: #c90000;
				}
		...
		wp_add_inline_style( 'hypebiz-theme-notice', $custom_style );
		wp_register_script(
			'hypebiz-theme-notice',
			'',
			array( 'wp-api-fetch' ),
			'1.0.0',
			true
		...
		wp_enqueue_script( 'hypebiz-theme-notice' );
		...
		function sequenceInstall(plugin, pluginsInstalled) {
			return new Promise((resolve, reject) => {
				...
				const needUpdate = plugin.installed
					? HYPEBIZPluginUtils.isVersionGreater(plugin.req_version, pluginsInstalled[`${path}.php`].Version)
					: false;
		...
		document.addEventListener('DOMContentLoaded', () => {
			const notice = document.querySelector('.notice.is-dismissible.hypebiz-simple-notice');
			...
			button.addEventListener('click', function (e) {
				...
				button.innerHTML = `<?php esc_html_e( 'Installing...', 'hypebiz' ); ?>`;
				...
				if (!hasFinishClass) {
					...
					sequence.then(() => {
						...
						button.innerHTML = `<?php esc_html_e( 'Install Failed, Try Again', 'hypebiz' ); ?>`;
		...
		wp_add_inline_script( 'hypebiz-theme-notice', $custom_script );

		?>
		<div class="notice notice-info is-dismissible hypebiz-simple-notice" data-nonce="<?php echo esc_attr( wp_create_nonce( "hypebiz_dismiss" ) ); ?>">
				<p>
					<strong class="hypebiz-notice-title"><?php esc_html_e( "Thank you For Installing Hypebiz Theme", "hypebiz" ); ?></strong>
					<span class="hypebiz-notice-description">
						<?php esc_html_e( "Unlock the full potential of your website with the recommended plugins.", "hypebiz" ); ?>
						<br/>
						<?php esc_html_e( "Activate it to explore exclusive extensions, ready-to-use demo templates, and powerful features that make building your site easier and more enjoyable.", "hypebiz" ); ?>
					</span>
				</p>
				<p>
					<a href="#" class="button button-primary" id="gutenverse-install-plugin"><?php echo esc_html__( "Install Recommended Plugins", "hypebiz" ); ?></a>
				...
				<p class="installing-warning" style="font-size: 12px; color: #666; display: none; margin-top: 5px;">
					<i><?php esc_html_e( "Don\'t refresh the page when installing recommended plugins", "hypebiz" ); ?></i>
				</p>
			</div>
		<?php
	}
	...
	public function dismiss_notice() {
		check_ajax_referer( 'hypebiz_dismiss' );

		if ( ! get_option( 'hypebiz_dismiss_notice' ) ) {
			update_option( 'hypebiz_dismiss_notice', true );
		}
